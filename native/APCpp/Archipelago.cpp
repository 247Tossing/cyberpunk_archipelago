    ...
    webSocket.setOnMessageCallback([](const ix::WebSocketMessagePtr& msg)
        {
            ...
            else if (msg->type == ix::WebSocketMessageType::Error || msg->type == ix::WebSocketMessageType::Close)
            {
                ...
                if (!msg->errorInfo.reason.empty()) {
                    last_connection_error = msg->errorInfo.reason;
                } else {
...
AP_ConnectionStatus AP_GetConnectionStatus() {
    ...
    if (webSocket.getReadyState() == ix::ReadyState::Connecting) {
        return AP_ConnectionStatus::Negotiating;
    }
...
const char* AP_GetLastConnectionError() {
    return last_connection_error.c_str();
}
...
bool parse_response(std::string msg, std::string &request) {
    ...
    for (unsigned int i = 0; i < root.size(); i++) {
        ...
        if (cmd == "RoomInfo") {
            ...
            refused = true;
            last_connection_error.clear();
            if (root[i].isMember("errors") && root[i]["errors"].isArray() && root[i]["errors"].size() > 0) {
                for (unsigned int j = 0; j < root[i]["errors"].size(); j++) {
                    if (j > 0) {
                        last_connection_error += ", ";
                    }
                    last_connection_error += root[i]["errors"][j].asString();
                }
            } else {
                last_connection_error = "Connection refused by the server.";
            }
            printf("AP: Archipelago Server has refused connection: %s\n", last_connection_error.c_str());
            fflush(stdout);
        } else if (cmd == "Bounced") {
